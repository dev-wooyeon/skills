...
AI를 활용하며 반복적으로 사용하는 스킬 중 유지 필요가 높은 스킬의
실제 본문을 보관하는 저장소입니다.

각 파일은 요약본이 아니라 실제 스킬 원문을 아래 형태로 감싸서 저장합니다.
...
- 로컬 `~/.codex/skills`의 실제 스킬 본문을 GitHub에서 버전 관리
- 스킬 이름과 로컬 경로를 함께 남겨 어떤 원본을 관리하는지 명확히 식별
...
- [engineering-analysis/double-check.md](engineering-analysis/double-check.md)
  - 실제 `double-check` 스킬 본문
- [engineering-analysis/problem-structuring.md](engineering-analysis/problem-structuring.md)
  - 실제 `problem-structuring` 스킬 본문
- [engineering-analysis/tradeoff-decision.md](engineering-analysis/tradeoff-decision.md)
  - 실제 `tradeoff-decision` 스킬 본문

## 관리 원칙

- 이 저장소는 요약 설명보다 실제 스킬 원문 보관을 우선합니다.
- 원본은 로컬 `~/.codex/skills/<skill-name>/SKILL.md`입니다.
- 로컬 스킬이 바뀌면 대응하는 GitHub 파일도 같이 갱신합니다.
- 파일명은 스킬 이름과 동일하게 유지해 검색성과 식별성을 높입니다.
